...
func (a *myProviderAdapter) SubscriptionInfo() map[string]int64 {
...
func (a *myProviderAdapter) ParseSubInfo(infoString string) bool {
	...
	if len(result) > 0 {
		...
		a.subscriptionInfo.upload = int64(upload)
		a.subscriptionInfo.download = int64(download)
		a.subscriptionInfo.total = int64(total)
		a.subscriptionInfo.expire = int64(expire)
		return true
	}
	return false
}
...
func (p *myProviderAdapter) BackupProvider() ([]adapter.Outbound, map[string]adapter.Outbound, SubscriptionInfo) {
	...
	for tag, out := range p.outboundByTag {
	...
	p.subscriptionInfo.upload = int64(0)
	p.subscriptionInfo.download = int64(0)
	p.subscriptionInfo.total = int64(0)
	p.subscriptionInfo.expire = int64(0)
...
func (p *myProviderAdapter) PRLock() {
	p.RLock()
}

func (p *myProviderAdapter) PRUnlock() {
	p.RUnlock()
}
